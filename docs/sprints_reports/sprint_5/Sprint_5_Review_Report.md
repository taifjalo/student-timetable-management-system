...
This includes externalizing all static text from the UI, implementing a language selection mechanism, and fully localizing the GUI to support Arabic (with RTL layout support) and Russian, in addition to the default English.
...
* All static UI text was externalized from controllers into three `ResourceBundle` `.properties` files:
    + `messages_en.properties` — English (default)
    + `messages_ar.properties` — Arabic
    + `messages_ru.properties` — Russian
* `LanguageManager.java` singleton service was created in `org.service`, handling locale switching, `ResourceBundle` loading, and RTL detection via `isRTL()`.
...
* A `refreshUI()` method was implemented in `MainAppController` to reload all UI text after a language switch without restarting the application.
...
The Arabic RTL layout was demonstrated, showing how the entire application layout mirrors correctly when Arabic is selected. The team also walked through the `LanguageManager` service and the `.properties` file structure to show how the localization framework is implemented.

## Team Member Contributions

| Team Member Name | Assigned Tasks | Time Spent (hrs) | In-class Tasks |
|---|---|---|---|
| Taif Jalo | LanguageManager service, SessionManager integration, Sprint reports | 8 | Submitted |
| Miska Voutilainen | messages_en/ar/ru.properties — all string translations | 7 | Submitted |
| Elias Norta | Language selector UI, UserSettingsController update, refreshUI() | 7 | Submitted |
| Nikita Rybakov | RTL support, NodeOrientation, layout adjustments across all screens | 6 | Submitted |
...
* The `LanguageManager` singleton made it easy to access translations from any controller
