...
- **Localization:** Java's built-in `java.util.ResourceBundle` is used for UI internationalization (i18n). Translation strings are stored in `.properties` files under `src/main/resources/i18n/MessagesBundle/`. The `LocalizationSevice` service handles runtime language switching and RTL layout detection for Arabic.
